...
//--------------------------------------------------------------------
/*
* get_crc8 - определени€ чек-суммы по алгоритму crc-8
*/
uint8_t get_crc8(uint8_t *addr, uint8_t len)
{
    uint8_t crc = 0;
		...
		if (USBDataRequest() == USBH_OK)
		{
			...
			if (USBTransmitted)
			{
				...
				if (USBReceived &&
					...
					(get_crc8(rx_buffer, (DataByteCnt + 4))) == (rx_buffer[(DataByteCnt + 4)])) {// провер€ем чексумму прин€того сообщени€, если не совпадает - шлем запрос заново
