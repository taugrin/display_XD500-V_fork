...
void LoadToDriveScreenDraw(void)
{
	...
	// Текст
	DrawStringWithAlign(2, ALIGN_CENTER, "КОПИРОВАНИЕ");
	DrawStringWithAlign(4, ALIGN_CENTER, "ПУЛЬТ -> ПРИВОД");
	ST7565_display();
	...
	while (i < AllGroupsCnt)
	{
		...
		if (AllGroups[i]->saveInEeprom) // группа сохраняется в eeprom
		{
			while (j < AllGroups[i]->paramCnt) // перебираем параметры группы
			{
				res = readParamFromEeprom(AllGroups[i]->params[j].adr, &readData);
				if (res)
				{
					UsbWriteReg(AllGroups[i]->params[j].adr, readData);
					err_cnt = 0;
					j++;
				}
				else
