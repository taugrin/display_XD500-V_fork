...
void InitBasicMenu(void)
{
	...
	// Добавляю элементы меню "ФУНКЦИИ КОПИРОВАНИЯ"
	MenuNextIdx = MenuItemsCnt+2;
	MenuItemsCnt = Menu_AddItem("ЗАВОДСКИЕ НАСТРОЙКИ", 3, MenuNextIdx, RestoreToDefaultScr); MenuFirstIdx = MenuItemsCnt;
	MenuNextIdx = MenuItemsCnt+2;
	MenuItemsCnt = Menu_AddItem("КОПИРОВАТЬ В ПУЛЬТ", 3, MenuNextIdx, CopyToPultScr);
	MenuItemsCnt = Menu_AddItem("ЗАГРУЗИТЬ В ПРИВОД", 3, MenuFirstIdx, LoadToDriveScr);
