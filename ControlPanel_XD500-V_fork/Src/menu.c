...
void InitBasicMenu(void)
{
	...
	Menu_AddItem("АРХИВ СОБЫТИЙ", -1, 2, NoScr); //[1]
	Menu_AddItem("БЫСТРЫЙ СТАРТ", -1, 3, NoScr); //[2]
	Menu_AddItem("ФУНКЦИИ КОПИРОВАНИЯ", -1, 4, NoScr); //[3]
	Menu_AddItem("ВЕРСИИ ПО", -1, 5, SoftVersionsScr); //[4]
	Menu_AddItem("МОНИТОРИНГ", -1, 6, NoScr); //[5]
	...
	// Добавляю элементы в меню МОНИТОРИНГ
	MenuNextIdx = MenuItemsCnt+2;
	MenuItemsCnt = Menu_AddItem("МОНИТОРИНГ 1", 5, MenuNextIdx, NoScr);
	MenuFirstIdx = MenuItemsCnt;
	MenuNextIdx = MenuItemsCnt+2;
	MenuItemsCnt = Menu_AddItem("МОНИТОРИНГ 2", 5, MenuNextIdx, NoScr);
	MenuNextIdx = MenuFirstIdx;
	MenuItemsCnt = Menu_AddItem("МОНИТОРИНГ 3", 5, MenuNextIdx, NoScr);
