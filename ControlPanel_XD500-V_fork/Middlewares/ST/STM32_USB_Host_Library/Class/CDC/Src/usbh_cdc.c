...
static void CDC_ProcessTransmission(USBH_HandleTypeDef *phost) {
  ...
  case CDC_SEND_DATA:
    if (CDC_Handle->TxDataLength > CDC_Handle->DataItf.OutEpSize)
      USBH_BulkSendData (phost, CDC_Handle->pTxData, CDC_Handle->DataItf.OutEpSize, CDC_Handle->DataItf.OutPipe, 1);
    else
      USBH_BulkSendData (phost, CDC_Handle->pTxData, CDC_Handle->TxDataLength, CDC_Handle->DataItf.OutPipe, 1);
  ...
  case CDC_SEND_DATA_WAIT:
    ...
    if (URB_Status == USBH_URB_DONE) {
      ...
      else CDC_Handle->TxDataLength = 0;
      
      if (CDC_Handle->TxDataLength > 0) CDC_Handle->data_tx_state = CDC_SEND_DATA;
      else {
